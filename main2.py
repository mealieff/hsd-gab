...
def main(args):
    ...
    for method_name, emb_file, label_file in methods:
        ...
        label_names = ["HD", "CV", "VO", "Other"][:num_labels] if args.labels == 4 else [f"Label_{i}" for i in range(num_labels)]

        # Manual F1 computation by label index
        num_labels = test_labels.shape[1]
        f1_scores = []
        ...
        for i in range(num_labels):
            ...
            print(f"[EVAL] Label {i}: Precision={precision:.4f}, Recall={recall:.4f}, F1={f1:.4f}")
